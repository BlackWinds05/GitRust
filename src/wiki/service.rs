...
pub async fn delete_page(pool: &PgPool, repo_id: Uuid, slug: &str) -> AppResult<()> {
    sqlx::query("DELETE FROM wiki_pages WHERE repository_id=$1 AND slug=$2")
        .bind(repo_id).bind(slug).execute(pool).await?;
    Ok(())
}

pub async fn search_pages(pool: &PgPool, repo_id: Uuid, query: &str) -> AppResult<Vec<WikiPageWithAuthor>> {
    let pattern = format!("%{}%", query);
    Ok(sqlx::query_as::<_, WikiPageWithAuthor>(
        r#"SELECT DISTINCT ON (wp.slug) wp.id, wp.title, wp.slug, wp.content, wp.revision,
                  u.username as author_username, u.display_name as author_display_name, wp.updated_at
           FROM wiki_pages wp JOIN users u ON wp.author_id = u.id
           WHERE wp.repository_id = $1 AND (wp.title ILIKE $2 OR wp.content ILIKE $2)
           ORDER BY wp.slug, wp.revision DESC"#,
    ).bind(repo_id).bind(&pattern).fetch_all(pool).await?)
}
